...
$peternakId = $row['ID_PETERNAK'];

$sql = "SELECT NO_KLOTER FROM penetasan WHERE ID_PETERNAK = '$peternakId' ORDER BY NO_KLOTER DESC LIMIT 1";
$result = mysqli_query($koneksi, $sql);
$row = mysqli_fetch_assoc($result);
$lastId = $row['NO_KLOTER'];
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="false">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder text-white mb-0" style="font-size: 30px;">Input Eggs</h6>
        </nav>
      </div>
    ...
    <div class="container-fluid py-4">
      <div class="row mt-4">
        <div class="col-lg-14 mb-lg-0 mb-4">
          <div class="card">
            <div class="card-body">
              <form action="prosesinputdata.php" method="post" enctype="multipart/form-data">
                <div class="form-group">
                  <label for="example-text-input" class="form-control-label h6">Group Id</label>
                  <input class="form-control bg-white" type="number" id="example-text-input NO_KLOTER" name="NO_KLOTER" placeholder="Group id" value="<?php echo $newId?>" required readonly>
                </div>
                <div class="form-group">
                  <label for="example-date-input" class="form-control-label h6">Date of entry</label>
                  <input class="form-control" type="date" id="example-date-input TGL_MASUK" name="TGL_MASUK" required>
                </div>
                <div class="form-group">
                  <label for="example-date-input" class="form-control-label h6">Hatch Predictions</label>
                  <input class="form-control" type="date" id="example-date-input PRED_MENETAS" name="PRED_MENETAS" required>
                </div>
                <div class="form-group">
                  <label for="example-text-input" class="form-control-label h6">Number of Eggs</label>
                  <input class="form-control" type="number" id="example-text-input JMLH_TELUR" name="JMLH_TELUR" placeholder="How much eggs" required>
                </div>
                <button type="submit" class="btn btn-warning btn-lg w-100" name="simpan" id="simpan" value="Simpan">
                  <span class="btn-inner--icon"><i class="ni ni-send"></i></span>
                  <span class="btn-inner--text">Send Data</span>
                </button>
              </form>
            </div>
          </div>
